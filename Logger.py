...
def init(logFilePath, logFileName):
    ...
    # Redirect output from C Modules to a log file (assumed to be
    # output from Metashape)
    metashapeLogFile = os.path.join(
        logFilePath, "{}_metashape.txt".format(logFileName))
    redirectCStdout(metashapeLogFile)

    # Read base logging config
    configFilePath = path.join(path.dirname(
    ...
    LF_Handler = logging.FileHandler(os.path.join(
        logFilePath, "{}_log.txt".format(logFileName)), "w")
    LF_Formatter = logging.Formatter(
        fmt="%(asctime)s - %(name)s - %(levelname)s - %(message)s", datefmt="")
...
def redirectCStdout(filename):
    ...
    try:
        ...
        # Duplicate stdout (usually file descriptor 1) to a different
        ...
        # Create a file and overwrite file descriptor 1 to be that then
        # close it
