...
def quickAlign(chunk):
    ...
    chunk.matchPhotos(downscale=2, generic_preselection=True, filter_mask=True,
                      mask_tiepoints=False, keypoint_limit=(40000),
                      tiepoint_limit=(4000))
...
def genAlign(chunk):
    ...
    chunk.matchPhotos(downscale=1, generic_preselection=True, filter_mask=True,
                      mask_tiepoints=False, keypoint_limit=(0),
                      tiepoint_limit=(0))
...
def arcAlign(chunk):
    ...
    #   the time added is negligible.
    logger.info("Quickly aligning photos")
    chunk.matchPhotos(downscale=0, generic_preselection=True, filter_mask=True,
                      mask_tiepoints=False, keypoint_limit=(0),
                      tiepoint_limit=(0))
...
def genDenseCloud(chunk):
    ...
    chunk.buildDenseCloud(
        point_colors=True, keep_depth=True, max_neighbors=100)
...
def arcDenseCloud(chunk):
    ...
    chunk.buildDepthMaps(quality=Metashape.HighestQuality,
                         filter=Metashape.MildFiltering, max_neighbors=100)
...
def arcModel(chunk):
    ...
    chunk.buildTexture(blending_mode=Metashape.MosaicBlending,
                       texture_size=(16384), fill_holes=False)
    logger.info("Done building model")
